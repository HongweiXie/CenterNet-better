...
from dl_lib.builder import build_detector,build_encoder
...
def main(args):
    ...
    model = build_detector(raw_cfg.detector)
    encoder = build_encoder(raw_cfg.encoder)
    config = raw_cfg.config
    ...
    for current_file in valid_files:
        ...
        res = Trainer.test(cfg, model, data_encoder=encoder)
